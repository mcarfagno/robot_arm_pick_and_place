'''
KR210 Inverse Kinematics Server
'''
...
def handle_calculate_IK(req):
    ...
    if len(req.poses)<1:
        ...
        for x in xrange(0, len(req.poses()):
            ...
            #Compute dsired Wrist Center Position rela
            ...
            side_a = 1.501
            side_b = sqrt(pow((sqrt(pos_wc[0]**2 + pos_wc[1]**2)-0.35),2) + pow((pos_wc[2]-0.75),2))
            side_c = 1.25

            angle_a = acos((side_b**2 + side_c**2 - side_a**2)/(2*side_b*side_c))
            angle_b = acos((side_a**2 + side_c**2 - side_b**2)/(2*side_a*side_c))
            angle_b = acos((side_a**2 + side_b**2 - side_c**2)/(2*side_a*side_b))

            theta2 = pi/2 - angle_a - atan2(pos_wc(2)-0.75, sqrt(pos_wc[0]**2 + pos_wc[1]**2)-0.35)
            theta3 = pi/2 - (angle_b + 0.036)

            #Sperical wrist joints angles
            #Extract Rotation Matrix of the arm from the Transformation Matrix
            R0_3 = T0_1[0:3,0:3]*T1_2[0:3,0:3]*T2_3[0:3,0:3]
            R0_3 = R0_3.evalf(subs={q1:theta1, q2:theta2, q3:theta3})

            #Rotation matrix from EE to wrist
            ...
            #(OPTIONAL) Evaluate Forward Kinematics
